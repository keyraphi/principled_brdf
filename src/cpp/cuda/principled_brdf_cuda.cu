...
#pragma unroll
  ...
  const Vec3 dBRDF_dP_c =
      Vec3{1.F, 1.F, 1.F} * 0.25F * G_r(L, V, n) * F_r(L, H) * D_r(H, n, P_cg);
  // write out result
  result[(3 * global_id)] = dBRDF_dP_c.x;
  result[(3 * global_id) + 1] = dBRDF_dP_c.y;
  result[(3 * global_id) + 2] = dBRDF_dP_c.z;
}
