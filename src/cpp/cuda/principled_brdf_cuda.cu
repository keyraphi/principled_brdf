...
#pragma unroll
  ...
  result[(9 * global_id) + 0] = dBRDFdP_b.m[0];
  result[(9 * global_id) + 1] = dBRDFdP_b.m[1];
  result[(9 * global_id) + 2] = dBRDFdP_b.m[2];
  result[(9 * global_id) + 3] = dBRDFdP_b.m[3];
  result[(9 * global_id) + 4] = dBRDFdP_b.m[4];
  result[(9 * global_id) + 5] = dBRDFdP_b.m[5];
  result[(9 * global_id) + 6] = dBRDFdP_b.m[6];
  result[(9 * global_id) + 7] = dBRDFdP_b.m[7];
  result[(9 * global_id) + 8] = dBRDFdP_b.m[8];
}
...
#pragma unroll
  ...
  result[(3 * global_id) + 0] = dBRDFdP_m.x;
  result[(3 * global_id) + 1] = dBRDFdP_m.y;
  result[(3 * global_id) + 2] = dBRDFdP_m.z;
}
...
#pragma unroll
  ...
  result[(3 * global_id) + 0] = dBRDF_dP_ss.x;
  result[(3 * global_id) + 1] = dBRDF_dP_ss.y;
  result[(3 * global_id) + 2] = dBRDF_dP_ss.z;
}
...
__global__ void principled_brdf_backward_n_kernel(
    const float *__restrict__ omega_i_, const float *__restrict__ omega_o_,
    const float *__restrict__ P_b_, const float *__restrict__ P_m_,
    const float *__restrict__ P_ss_, const float *__restrict__ P_s_,
    const float *__restrict__ P_r_, const float *__restrict__ P_st_,
    const float *__restrict__ P_ani_, const float *__restrict__ P_c_,
    const float *__restrict__ P_cg_, const float *__restrict__ n_,
    float *result, size_t N) {
  uint32_t global_id = threadIdx.x + blockDim.x * blockIdx.x;
...
#pragma unroll
  ...
  const Mat3x3 nabla_n_BRDF =
      (1 - P_m) * (M_1_PIf * C_dlin(P_b).odot(nabla_n_mix(L, V, H, P_ss,
                                                              P_r, n))) +
      F_s(L, H, P_b, P_m, P_st, P_s)
          .odot((D_s(H, n, P_ani, P_r) *
                 nabla_n_G_s(L, V, P_r, P_ani, n)) +
                G_s(L, V, n, P_ani, P_r) *
                    nabla_n_D_s(H, P_r, P_ani, n)) +
      0.25F * P_c * F_r(L, H) *
          Vec3{1.F, 1.F, 1.F}.odot(D_r(H, n, P_cg) * nabla_n_G_r(L, V, n) +
                                   G_r(L, V, n) * nabla_n_D_r(H, P_cg, n));
  // write out result
  result[(9 * global_id) + 0] = nabla_n_BRDF.m[0];
  result[(9 * global_id) + 1] = nabla_n_BRDF.m[1];
  result[(9 * global_id) + 2] = nabla_n_BRDF.m[2];
  result[(9 * global_id) + 3] = nabla_n_BRDF.m[3];
  result[(9 * global_id) + 4] = nabla_n_BRDF.m[4];
  result[(9 * global_id) + 5] = nabla_n_BRDF.m[5];
  result[(9 * global_id) + 6] = nabla_n_BRDF.m[6];
  result[(9 * global_id) + 7] = nabla_n_BRDF.m[7];
  result[(9 * global_id) + 8] = nabla_n_BRDF.m[8];
}

extern "C" void principled_brdf_backward_n_cuda_impl(
    const float *__restrict__ omega_i, const float *__restrict__ omega_o,
    const float *__restrict__ P_b, const float *__restrict__ P_m,
    const float *__restrict__ P_ss, const float *__restrict__ P_s,
    const float *__restrict__ P_r, const float *__restrict__ P_st,
  ...
  // shared memory for coalesced read in of float3s
  const size_t shared_memory_bytes = sizeof(float) * 12 * N;
  principled_brdf_backward_n_kernel<<<N_BLOCKS, N_THREADS,
                                      shared_memory_bytes>>>(
      omega_i, omega_o, P_b, P_m, P_ss, P_s, P_r, P_st, P_ani, P_c, P_cg, n,
      result, N);
  // DEBUGGING
  // Synchronize and check for errors
  cudaError_t err = cudaDeviceSynchronize();
...
__global__ void principled_brdf_backward_L_kernel(
    const float *__restrict__ omega_i_, const float *__restrict__ omega_o_,
    const float *__restrict__ P_b_, const float *__restrict__ P_m_,
    const float *__restrict__ P_ss_, const float *__restrict__ P_s_,
    const float *__restrict__ P_r_, const float *__restrict__ P_st_,
...
#pragma unroll
  ...
  const Vec3 H = (V + L).normalize();
    ...
    const Mat3x3 nabla_L_BRDF =
        (1.F - P_m) * (M_1_PI * C_dlin(P_b).odot(nabla_L_mix(
                                       L, V, H, P_ss, P_r, n)) +
                          nabla_L_F_sheen(L, V, H, P_b, P_sh, P_sht)) +
        F_s(L, H, P_b, P_m, P_st, P_s)
            .odot(D_s(H, n, P_ani, P_r) *
                      nabla_L_G_s(L, V, P_r, P_ani, n) +
                  G_s(L, V, n, P_ani, P_r) *
                      nabla_L_D_s(L, V, H, P_r, P_ani, n)) +
        0.25F * P_c * F_r(L, H) *
            Vec3{1.F, 1.F, 1.F}.odot(
                D_r(H, n, P_c) * nabla_L_G_r(L, V, n) +
                G_r(L, V, n) * nabla_L_D_r(L, V, H, P_cg, n));

  // write out result
  result[(9 * global_id) + 0] = nabla_L_BRDF.m[0];
  result[(9 * global_id) + 1] = nabla_L_BRDF.m[1];
  result[(9 * global_id) + 2] = nabla_L_BRDF.m[2];
  result[(9 * global_id) + 3] = nabla_L_BRDF.m[3];
  result[(9 * global_id) + 4] = nabla_L_BRDF.m[4];
  result[(9 * global_id) + 5] = nabla_L_BRDF.m[5];
  result[(9 * global_id) + 6] = nabla_L_BRDF.m[6];
  result[(9 * global_id) + 7] = nabla_L_BRDF.m[7];
  result[(9 * global_id) + 8] = nabla_L_BRDF.m[8];
}

extern "C" void principled_brdf_backward_L_cuda_impl(
    const float *__restrict__ omega_i, const float *__restrict__ omega_o,
    const float *__restrict__ P_b, const float *__restrict__ P_m,
    const float *__restrict__ P_ss, const float *__restrict__ P_s,
    const float *__restrict__ P_r, const float *__restrict__ P_st,
  ...
  // shared memory for coalesced read in of float3s
  const size_t shared_memory_bytes = sizeof(float) * 12 * N;
  principled_brdf_backward_L_kernel<<<N_BLOCKS, N_THREADS, shared_memory_bytes>>>(
      omega_i, omega_o, P_b, P_m, P_ss, P_s, P_r, P_st, P_ani, P_sh, P_sht, P_c,
      P_cg, n, result, N);
  // DEBUGGING
  // Synchronize and check for errors
...
__global__ void principled_brdf_backward_V_kernel(
    const float *__restrict__ omega_i_, const float *__restrict__ omega_o_,
    const float *__restrict__ P_b_, const float *__restrict__ P_m_,
    const float *__restrict__ P_ss_, const float *__restrict__ P_s_,
    const float *__restrict__ P_r_, const float *__restrict__ P_st_,
...
#pragma unroll
  ...
  const Vec3 H = (V + L).normalize();
    ...
    const Mat3x3 nabla_V_BRDF =
        (1.F - P_m) * (M_1_PI * C_dlin(P_b).odot(nabla_V_mix(
                                       L, V, H, P_ss, P_r, n)) +
                          nabla_V_F_sheen(L, V, H, P_b, P_sh, P_sht)) +
        F_s(L, H, P_b, P_m, P_st, P_s)
            .odot(D_s(H, n, P_ani, P_r) *
                      nabla_V_G_s(L, V, P_r, P_ani, n) +
                  G_s(L, V, n, P_ani, P_r) *
                      nabla_V_D_s(L, V, H, P_r, P_ani, n)) +
        G_s(L, V, n, P_ani, P_r) * D_s(H, n, P_ani, P_r) *
            nabla_V_F_s(L, V, H, P_b, P_m, P_st, P_s) +
        0.25F * P_c *
            Vec3{1.F, 1.F, 1.F}.odot(
                F_r(L, H) *
                    (D_r(H, n, P_c) * nabla_V_G_r(L, V, n) +
                     G_r(L, V, n) * nabla_V_D_r(L, V, H, P_cg, n)) +
                G_r(L, V, n) * D_r(H, n, P_cg) * nabla_V_F_r(L, V, H));

  // write out result
  result[(9 * global_id) + 0] = nabla_V_BRDF.m[0];
  result[(9 * global_id) + 1] = nabla_V_BRDF.m[1];
  result[(9 * global_id) + 2] = nabla_V_BRDF.m[2];
  result[(9 * global_id) + 3] = nabla_V_BRDF.m[3];
  result[(9 * global_id) + 4] = nabla_V_BRDF.m[4];
  result[(9 * global_id) + 5] = nabla_V_BRDF.m[5];
  result[(9 * global_id) + 6] = nabla_V_BRDF.m[6];
  result[(9 * global_id) + 7] = nabla_V_BRDF.m[7];
  result[(9 * global_id) + 8] = nabla_V_BRDF.m[8];
}

extern "C" void principled_brdf_backward_V_cuda_impl(
    const float *__restrict__ omega_i, const float *__restrict__ omega_o,
    const float *__restrict__ P_b, const float *__restrict__ P_m,
    const float *__restrict__ P_ss, const float *__restrict__ P_s,
    const float *__restrict__ P_r, const float *__restrict__ P_st,
  ...
  // shared memory for coalesced read in of float3s
  const size_t shared_memory_bytes = sizeof(float) * 12 * N;
  principled_brdf_backward_V_kernel<<<N_BLOCKS, N_THREADS, shared_memory_bytes>>>(
      omega_i, omega_o, P_b, P_m, P_ss, P_s, P_r, P_st, P_ani, P_sh, P_sht, P_c,
      P_cg, n, result, N);
  // DEBUGGING
  // Synchronize and check for errors
